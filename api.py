...
class PetFriends:
    ...
    def get_api_key(self, email: str, passwd: str) -> json:
        ...
        JSON с уникальным ключом пользователя auth_key, найденным+ по указанным email и паролю"""
    ...
    def add_pet_photo(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        '''Метод добавляет фото к существующему питомцу с pet_id'''
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
            })
        # Заголовки запроса передаем в виде словаря
        headers = {'auth_key': auth_key['key'], 'Content-Type': data.content_type}

        # Сохраняем ответ запроса post в переменную res
